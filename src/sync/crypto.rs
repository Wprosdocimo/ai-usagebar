...
mod tests {
    ...
    fn only_the_crypto_module_imports_the_cryptographic_crates() {
        ...
        // **The floor is what this PR contains, and it must rise as the rest
        // lands.** These guards were written against the whole `src/sync`
        // tree; this branch carries the format modules only, so a `>= 20`
        // non-vacuity count would fail here for the right reason and be
        // silenced for the wrong one. Raising it with each staged PR is the
        // point — a guard that asserts an absence has to prove it looked.
        assert_eq!(skipped, 1, "crypto.rs must be excluded exactly once");
        assert!(checked >= 6, "only {checked} files walked under src/sync");
    }
}
